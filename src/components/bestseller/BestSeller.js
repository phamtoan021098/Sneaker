...
class BestSeller extends Component {
    ...
    render() {
        ...
        for (var i = 0; i < sneakers.length; i++) {
            if (sneakers[i].quantity <= 95) {
                currentbestseller.push(sneakers[i]);
            }
        }
        console.log(currentbestseller);
        currentbestseller.sort((a, b) => {
        ...
        const current = currentbestseller.slice(indexOfFirst, indexOfLast);
        ...
        const pageNumbers = [];
        for (var i = 1; i <= Math.ceil(currentbestseller.length / this.state.perpage); i++) {
            pageNumbers.push(i);
        }
